...
- `bedfile` the transcript annotation file for your organims of interest in *.bed* format. This should be the same genome assembly and annotation as the *.gtf* file and can be a bit tricky to find. For human GENCODE annotations the [UCSC Table Browser](https://genome.ucsc.edu/cgi-bin/hgTables?hgsid=1650209460_0G3ATy4TF3Sa3DTp0Cts3VMkDxe3&clade=mammal&org=Human&db=hg38&hgta_group=genes&hgta_track=fixSeqLiftOverPsl&hgta_table=0&hgta_regionType=genome&position=chr2%3A25%2C160%2C915-25%2C168%2C903&hgta_outputType=bed&hgta_outFileName=) provides a download portal. In theory, *.gtf* files can be converted to *.bed* format but I have not explored this deeply.
